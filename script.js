document.getElementById('processButton').addEventListener('click', () => {
    ...
    const globalContactName = document.getElementById('contactNameInput').value.trim();
...
function generateDownloadLink(file, newFileName, globalContactName, listItem) {
    ...
    reader.onload = () => {
        ...
        const baseFileName = newFileName.replace(/\.vcf$/, ''); // tanpa ekstensi
        ...
        const vcfContent = txtContent
            ...
            .map(line => {
                ...
                if (/^\d+$/.test(contact)) {
                    // Nama kontak = Global + FileRename + Nomor urut
                    const contactName = `${globalContactName} ${baseFileName} ${localCounter}`;
                    const fullContactName = currentCategory === 'Anggota'
                        ? contactName
                        : `${contactName} (${currentCategory})`;
